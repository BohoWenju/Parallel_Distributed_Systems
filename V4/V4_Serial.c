...
u_int32_t* p_indices,u_int32_t* p_indptr,u_int32_t* p_data){
  ...
  for (int i=0; i<M; i++){
    ...
    for (u_int32_t j=0; j<temp; j++){
      ...
      u_int32_t sum=binary_match(indices+indptr[i],indices+indptr[row],temp,temp1);
      if(sum>0){
        temp2++;
        p_data[previous]=sum;
...
int main(int argc, char *argv[]){
  ...
  u_int32_t new_size=Hadamard_squared(indices,indptr,M,indices_H,indptr_H,dataH);
